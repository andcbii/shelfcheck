...
test("release notes heading matches the package version", () => {
  const notesVersion = releaseNotes.match(/^#\s+Shelfcheck\s+([^\s]+)/m)?.[1];
  assert.equal(notesVersion, packageJson.version);
});
